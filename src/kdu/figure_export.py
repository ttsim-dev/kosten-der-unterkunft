"""Render a Plotly figure to the static image the presentation deck embeds.

Every figure this project writes as interactive HTML is also written as a static
image next to it, so that a slide renders the figure without loading Plotly and so that
the deck exports to PDF. This module is the single place that decides at what
size and resolution such an image is made.

Charts are written as PNG. The Gemeinde maps are written as JPEG: a choropleth of
some eleven thousand differently coloured areas barely compresses as PNG, and at
full resolution each map would carry several megabytes into the exported PDF.
"""
...
# The rendering geometry of a presentation image. A Slidev slide is 980 by 552 CSS
...
def write_presentation_image(figure: go.Figure, path: Path) -> None:
    """Write `figure` to `path` as an image sized for a 16:9 slide.
    ...
    The format follows the suffix of `path`, `.png` or `.jpg`.
