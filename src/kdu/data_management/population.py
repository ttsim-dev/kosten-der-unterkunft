...
from kdu.config import GEMEINDE_SIZE_CLASS_BREAKS, SMALL_GEMEINDE_THRESHOLD
...
def assign_size_class(population: pd.Series) -> pd.Series:
    """Bin population into the ordered Gemeindegrößenklassen of §8.3.

    The bins are left-closed and right-open at
    `kdu.config.GEMEINDE_SIZE_CLASS_BREAKS`.
    """
    breaks = (0, *GEMEINDE_SIZE_CLASS_BREAKS, float("inf"))
    labels = _size_class_labels(GEMEINDE_SIZE_CLASS_BREAKS)
    return pd.cut(population, bins=breaks, labels=labels, right=False)
...
def _finalise(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    result["gemeinde_size_class"] = assign_size_class(result["population"])
    result["is_small_gemeinde"] = result["population"] < SMALL_GEMEINDE_THRESHOLD
